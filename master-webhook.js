...
exports.handler = async function(event) {
 ...
 let stripeEvent;
 try {
 stripeEvent = stripe.webhooks.constructEvent(event.body, sig, process.env.STRIPE_WEBHOOK_ } catch (e) { return { statusCode: 400, body: `Webhook Error: ${e.message}` }; }
 ...
 if (meta.postingId) {
 const { data: posting } = await supabase.from('postings').select('*').eq('id', meta.pos await supabase.from('postings').update({
...
email: meta.email.toLowerCase().trim(),
 ...
 if (customer.email) {
 await supabase.from('poster_members').update({ active: false }).eq('email', customer.em console.log('Poster subscription cancelled:', customer.email);
 }
 }
