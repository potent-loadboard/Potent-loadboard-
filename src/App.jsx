...
// ── CONFIG ────────────────────────────────────────────────────────
const VEHICLES = [‘16ft Box Truck’,‘20ft Box Truck’,‘24ft Box Truck’,‘26ft Box Truck’,‘Cargo Van’,‘Sprinter Van’,‘Cargo SUV / Minivan’]
...
// ── LOAD DETAIL ───────────────────────────────────────────────────
// Reasonable max payload by vehicle type — flags when the stated weight
// looks too heavy for the vehicle the poster selected, so a driver isn’t
// surprised on arrival and a poster catches their own mistake early.
const VEHICLE_MAX_LBS={‘Cargo SUV / Minivan’:500,‘Cargo Van’:2500,‘Sprinter Van’:5000,‘16ft Box Truck’:4500,‘20ft Box Truck’:7000,‘24ft Box Truck’:8500,‘26ft Box Truck’:10000}
function weightMismatch(load){
const max=VEHICLE_MAX_LBS[load.vehicle]
if(!max||!load.weight)return null
const num=parseInt(String(load.weight).replace(/[^0-9]/g,’’),10)
if(!num||isNaN(num))return null
if(num>max)return {stated:num,max}
return null
}

function MessageThread({postingId, currentEmail, currentType, currentName}){
const [msgs,setMsgs]=useState(null)
const [reply,setReply]=useState(’’)
const [sending,setSending]=useState(false)

function refetch(){
supabase.from(‘messages’).select(’*’).eq(‘posting_id’,postingId).order(‘created_at’,{ascending:true})
.then(({data})=>setMsgs(data||[]))
.catch(()=>setMsgs([]))
}
useEffect(()=>{ refetch() },[postingId])

async function send(){
if(!reply.trim())return
setSending(true)
try{
const res=await fetch(’/api/send-message’,{method:‘POST’,headers:{‘Content-Type’:‘application/json’},body:JSON.stringify({postingId,senderEmail:currentEmail,senderName:currentName||’’,message:reply,senderType:currentType})})
...
if(msgs===null)return <div style={{fontSize:12,color:C.dim,padding:‘10px 0’}}>Loading messages…</div>

return(
<div style={{marginTop:14}}>
<div style={{fontSize:12,fontWeight:800,color:C.orange,marginBottom:8}}>💬 Messages</div>
<div style={{maxHeight:260,overflowY:‘auto’,marginBottom:10,border:`1px solid ${C.border}`,borderRadius:8,padding:8}}>
{msgs.length===0&&<div style={{fontSize:12,color:C.dim,textAlign:‘center’,padding:16}}>No messages yet — start the conversation.</div>}
{msgs.map(m=>{
const isMine=m.sender_email?.toLowerCase().trim()===currentEmail?.toLowerCase().trim()
return(
<div key={m.id} style={{display:‘flex’,justifyContent:isMine?‘flex-end’:‘flex-start’,marginBottom:8}}>
<div style={{maxWidth:‘80%’,background:isMine?C.orange+‘22’:C.card,border:`1px solid ${isMine?C.orange+'44':C.border}`,borderRadius:10,padding:‘8px 12px’}}>
<div style={{fontSize:9,color:C.dim,marginBottom:3,fontWeight:700}}>{m.sender_type===‘poster’?‘POSTER’:‘DRIVER’} · {m.sender_name||m.sender_email?.split(’@’)[0]}</div>
<div style={{fontSize:13,color:C.white}}>{m.message}</div>
</div>
</div>
)
})}
...
<input value={reply} onChange={e=>setReply(e.target.value)} placeholder=“Type a reply…” style={inp({flex:1,padding:‘9px 12px’,fontSize:13})} onKeyDown={e=>{if(e.key===‘Enter’)send()}}/>
<Btn onClick={send} disabled={sending||!reply.trim()} style={{padding:‘9px 16px’,fontSize:12}}>{sending?’…’:‘Send’}</Btn>
</div>
</div>
)
}
...
const s=scoreFor(rate)
const dl=daysLeft(load.expires_at)
const mismatch=weightMismatch(load)
...
```
  ...
  {mismatch&&<div style={{background:'#1a0000',border:'1px solid #E53E3E66',borderRadius:8,padding:'10px 14px',marginBottom:16,fontSize:12,color:'#E53E3E'}}>
    ⚠ Stated weight ({mismatch.stated.toLocaleString()} lbs) may be too heavy for a {load.vehicle} (typical max ~{mismatch.max.toLocaleString()} lbs). Confirm with the poster before committing.
  </div>}
  ...
  {hasAccess?(
    <Card>
      ...
      <MessageThread postingId={load.id} currentEmail={driverEmail} currentType="driver" currentName={msg.name}/>
    </Card>
  ):(
    <div style={{background:C.orange+'10',border:`2px solid ${C.orange}`,borderRadius:12,padding:'18px',textAlign:'center',marginBottom:16}}>
      ...
      <div style={{fontSize:12,color:C.dim,marginBottom:14,lineHeight:1.7}}>30 days FREE then $20/month. Cancel before trial ends and you won't be charged.</div>
      <Btn onClick={onNeedAccess} style={{padding:'11px 28px'}}>🎁 Start Free 30-Day Trial</Btn>
    </div>
  )}
...
try{
  ...
  // First post free — check if this poster (by email) has ever paid for a posting before.
  // If this is genuinely their first time, skip checkout entirely for this one post.
  let isFirstPost = false
  if(poster.email){
    const {data:priorPaid}=await supabase.from('postings').select('id').eq('contact_email',poster.email).eq('paid',true).limit(1)
    isFirstPost = !priorPaid || priorPaid.length===0
  }

  if(isFirstPost && plan!=='unlimited'){
    // Mark this posting paid directly — no Stripe, genuinely free first post.
    await supabase.from('postings').update({paid:true}).eq('id',postingId)
    setUploading(false)
    window.location.href=`?payment=success&posting=${postingId}&firstpost=1`
    return
  }
...
try{
// Log TOS agreement
await fetch(’/api/record-tos’,{method:‘POST’,headers:{‘Content-Type’:‘application/json’},body:JSON.stringify({email:email.toLowerCase().trim(),userType:‘driver’})}).catch(()=>{});
...
<div style={{fontSize:12,color:C.dim,marginBottom:20,lineHeight:1.7}}>30 days FREE then $20/month. See phone and email on every load. Cancel anytime before trial ends — no charge.</div>
...
```
  ...
  <Card style={{background:C.orange+'08',borderColor:C.orange+'44'}}>
    <div style={{fontSize:22,fontWeight:900,color:C.orange,textAlign:'center',marginBottom:4}}>🎁 30 Days FREE<br/>then $20/mo</div>
    <div style={{fontSize:11,color:C.dim,textAlign:'center',marginBottom:12}}>Cancel anytime before your trial ends — no charge.</div>
    {['Direct phone & email for every poster','Message posters through the platform','See all loads including new ones as posted','Rate scoring — know if a load is worth it'].map(i=><div key={i} style={{display:'flex',gap:8,fontSize:12,color:C.dim,padding:'3px 0'}}><span style={{color:C.green}}>✓</span>{i}</div>)}
  </Card>
  ...
  <Btn onClick={subscribe} disabled={loading||!email} style={{width:'100%',padding:'13px',fontSize:14}}>
    {loading?'Redirecting...':"Start Free 30-Day Trial"}
  </Btn>
</div>
```
...
<MessageThread postingId={p.id} currentEmail={p.contact_email} currentType="poster" currentName={p.poster_name}/>
</Card>
})}
</div>
)
...
```
  ...
  {tab==='drivers'&&<div>
    <div style={{display:'grid',gridTemplateColumns:'repeat(2,1fr)',gap:10,marginBottom:12}}>
      ...
      <Card style={{padding:'12px',textAlign:'center',marginBottom:0}}><div style={{fontSize:10,color:C.dim}}>MONTHLY REVENUE</div><div style={{fontSize:24,fontWeight:900,color:C.orange}}>${(activeDriver.length*DRIVER_FEE).toLocaleString()}</div></Card>
    </div>
    {data.drivers?.map(d=>(
...
const tr=params.get(‘truck’),ds=params.get(‘dispute’)
if(tr===‘success’)setBanner({type:‘success’,text:‘Truck listed! 30 days free then $20/mo.’})
if(tr===‘cancelled’)setBanner({type:‘warn’,text:‘Truck listing cancelled. No charge.’})
if(ds===‘filed’)setBanner({type:‘success’,text:‘Dispute filed. POTENT will review within 48 hours.’})
if(params.get(‘dispute’)===‘filed’)setBanner({type:‘success’,text:‘Dispute filed. POTENT will review and respond within 48 hours.’})
...
```
  ...
  <div style={{borderBottom:`1px solid ${C.border}`,padding:'10px 16px',display:'flex',justifyContent:'space-between',alignItems:'center',gap:8,flexWrap:'wrap'}}>
    ...
    <div style={{display:'flex',gap:6,alignItems:'center',flexWrap:'wrap'}}>
      ...
      {navBtn('🚛 Trucks','trucks',view==='trucks')}
      {navBtn('📄 BOL','bol',view==='bol')}
    </div>
  </div>
  ...
  {view==='trucks'&&<TrucksBoard driverEmail={driverEmail} onBack={()=>setView('board')}/>}
  {view==='bol'&&<BOLGenerator onBack={()=>setView('board')}/>}
  {view==='tos'&&<TOSPage onBack={()=>setView('board')}/>}}

  {view==='board'&&!openLoad&&<div style={{maxWidth:860,margin:'0 auto',padding:'12px 16px 60px'}}>
    {/* No-commitment hook — the actual thing that should remove hesitation, made prominent */}
    <div style={{background:C.orange+'12',border:`1px solid ${C.orange}`,borderRadius:10,padding:'12px 16px',marginBottom:14,textAlign:'center'}}>
      <div style={{fontSize:15,fontWeight:900,color:C.orange}}>🎁 No commitment — 30 days completely free, both sides</div>
      <div style={{fontSize:11,color:C.dim,marginTop:2}}>Cancel anytime before your trial ends. You're not locked into anything by signing up.</div>
    </div>
    {/* Value prop — compact */}
    <div style={{borderBottom:`1px solid ${C.border}`,paddingBottom:10,marginBottom:12}}>
      <div style={{display:'flex',gap:16,flexWrap:'wrap',alignItems:'center'}}>
        <div style={{fontSize:11,color:C.dim}}>
          ...
          Drivers get 30 days free then $20/mo.
  ...
  <div style={{textAlign:'center',fontSize:10,color:C.faint,padding:'20px 0 40px',borderTop:`1px solid ${C.border}`}}>
    POTENT LOADBOARD · A POTENT PRÄDƏKT® Product · © 2026 ANTHONY EMMANUEL FIGUEROA MENDES® · <span onClick={()=>setView('tos')} style={{cursor:'pointer',color:C.orange,textDecoration:'underline'}}>Terms of Service</span>
...
// ═══════════════════════════════════════════════════════════════════
// 🚛 AVAILABLE TRUCKS BOARD
// ═══════════════════════════════════════════════════════════════════
function TrucksBoard({driverEmail,onBack}){
const [trucks,setTrucks]=useState([])
const [loading,setLoading]=useState(true)
const [view,setView]=useState(‘list’) // list | post
const [form,setForm]=useState({name:’’,email:’’,phone:’’,company:’’,truckType:‘Box Truck’,capacity:‘4300’,homeCity:’’,homeState:‘GA’,willingToGo:’’,availableFrom:’’,availableTo:’’,notes:’’})
const [submitting,setSubmitting]=useState(false)
const [msg,setMsg]=useState(’’)

useEffect(()=>{
setLoading(true)
supabase.from(‘available_trucks’).select(’*’).eq(‘paid’,true).eq(‘active’,true).eq(‘banned’,false).order(‘created_at’,{ascending:false})
...
e.preventDefault()
if(!form.email||!form.homeCity||!form.homeState){setMsg(‘Email, city, and state required.’);return}
setSubmitting(true);setMsg(’’)
const res=await fetch(’/.netlify/functions/post-truck’,{method:‘POST’,headers:{‘Content-Type’:‘application/json’},body:JSON.stringify(form)})
const data=await res.json()
if(data.url){window.location.href=data.url}
else{setMsg(‘Something went wrong. Try again.’);setSubmitting(false)}
}
...
<button onClick={()=>setView(‘list’)} style={{background:‘none’,border:‘none’,color:C.dim,cursor:‘pointer’,fontSize:13,fontFamily:ff}}>← Back</button>
<div style={{fontSize:16,fontWeight:900,color:C.white}}>List Your Truck</div>
</div>
<div style={{background:C.card,border:`1px solid ${C.orange}44`,borderRadius:10,padding:‘12px 14px’,marginBottom:16,fontSize:11,color:C.orange}}>
🎁 <strong>30 days FREE</strong> then $20/month. Cancel anytime before trial ends — no charge.
</div>
<div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:10,padding:‘14px’,marginBottom:10,fontSize:10,color:C.dim,lineHeight:1.7}}>
⚠ By listing your truck you agree to our <span onClick={()=>setView(‘tos’)} style={{color:C.orange,cursor:‘pointer’,textDecoration:‘underline’}}>Terms of Service</span>. POTENT is not responsible for any transactions between parties. All fees are final. No refunds.
</div>
<form onSubmit={handlePost}>
{[[‘name’,‘Your Name’,‘text’],[‘email’,‘Email Address’,‘email’],[‘phone’,‘Phone Number’,‘tel’],[‘company’,‘Company (optional)’,‘text’]].map(([k,l,t])=>(
<div key={k} style={{marginBottom:10}}>
<div style={{fontSize:9,color:C.dim,textTransform:‘uppercase’,marginBottom:3}}>{l}</div>
<input type={t} value={form[k]} onChange={e=>setForm(p=>({…p,[k]:e.target.value}))} required={k!==‘company’} style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:7,color:C.white,padding:‘9px 12px’,fontSize:13,width:‘100%’,outline:‘none’,fontFamily:ff,boxSizing:‘border-box’}}/>
</div>
))}
<div style={{marginBottom:10}}>
<div style={{fontSize:9,color:C.dim,textTransform:‘uppercase’,marginBottom:3}}>Truck Type</div>
<select value={form.truckType} onChange={e=>setForm(p=>({…p,truckType:e.target.value}))} style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:7,color:C.white,padding:‘9px 12px’,fontSize:13,width:‘100%’,outline:‘none’,fontFamily:ff,colorScheme:‘dark’}}>
{[‘Box Truck’,‘Cargo Van’,‘Sprinter Van’,‘Cargo SUV/Minivan’].map(v=><option key={v}>{v}</option>)}
</select>
</div>
<div style={{display:‘grid’,gridTemplateColumns:‘1fr 1fr’,gap:‘0 10px’}}>
{[[‘capacity’,‘Max Weight (lbs)’,‘number’],[‘homeCity’,‘Home City’,‘text’],[‘homeState’,‘State’,‘text’],[‘willingToGo’,‘Willing to go (states)’,‘text’],[‘availableFrom’,‘Available From’,‘date’],[‘availableTo’,‘Available To’,‘date’]].map(([k,l,t])=>(
<div key={k} style={{marginBottom:10}}>
<div style={{fontSize:9,color:C.dim,textTransform:‘uppercase’,marginBottom:3}}>{l}</div>
<input type={t} value={form[k]} onChange={e=>setForm(p=>({…p,[k]:e.target.value}))} style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:7,color:C.white,padding:‘9px 12px’,fontSize:13,width:‘100%’,outline:‘none’,fontFamily:ff,boxSizing:‘border-box’,colorScheme:‘dark’}}/>
</div>
))}
</div>
<div style={{marginBottom:14}}>
<div style={{fontSize:9,color:C.dim,textTransform:‘uppercase’,marginBottom:3}}>Additional Notes</div>
<textarea value={form.notes} onChange={e=>setForm(p=>({…p,notes:e.target.value}))} rows={3} placeholder=“Liftgate available, white glove, hazmat certified…” style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:7,color:C.white,padding:‘9px 12px’,fontSize:13,width:‘100%’,outline:‘none’,fontFamily:ff,boxSizing:‘border-box’,resize:‘vertical’}}/>
</div>
{msg&&<div style={{color:C.red,fontSize:11,marginBottom:10}}>{msg}</div>}
<button type=“submit” disabled={submitting} style={{width:‘100%’,background:submitting?C.surface:C.orange,color:submitting?C.dim:’#000’,border:‘none’,borderRadius:8,padding:‘13px’,fontSize:14,fontWeight:700,cursor:submitting?‘not-allowed’:‘pointer’,fontFamily:ff}}>
{submitting?‘Redirecting…’:‘🚛 List My Truck — Free 30-Day Trial’}
</button>
</form>
</div>
)
...
<div>
<div style={{fontSize:18,fontWeight:900,color:C.white}}>🚛 Available Trucks</div>
<div style={{fontSize:11,color:C.dim,marginTop:2}}>Box trucks, vans, and cargo vehicles available for loads</div>
</div>
<Btn onClick={()=>setView(‘post’)} style={{fontSize:12,padding:‘8px 16px’}}>+ List My Truck — 30 Days Free</Btn>
</div>
{loading&&<div style={{textAlign:‘center’,padding:40,color:C.dim}}>Loading…</div>}
{!loading&&trucks.length===0&&<div style={{textAlign:‘center’,padding:60,color:C.dim,fontSize:13}}>
No trucks listed yet. Be the first — list yours free for 30 days.
<br/><br/>
<Btn onClick={()=>setView(‘post’)} style={{fontSize:13,padding:‘10px 20px’}}>List My Truck Free</Btn>
</div>}
{trucks.map(t=>(
<div key={t.id} style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:10,padding:‘14px 16px’,marginBottom:8}}>
<div style={{display:‘flex’,justifyContent:‘space-between’,alignItems:‘flex-start’,gap:12}}>
<div style={{flex:1}}>
<div style={{display:‘flex’,gap:8,alignItems:‘center’,marginBottom:6,flexWrap:‘wrap’}}>
<span style={{fontSize:13,fontWeight:700,color:C.white}}>{t.truck_type}</span>
{t.capacity_lbs&&<span style={{fontSize:10,background:C.surface,color:C.dim,borderRadius:5,padding:‘2px 7px’}}>{Number(t.capacity_lbs).toLocaleString()} lbs max</span>}
<span style={{fontSize:10,background:C.orange+‘18’,color:C.orange,borderRadius:5,padding:‘2px 7px’,fontWeight:700}}>✅ Available</span>
</div>
<div style={{fontSize:12,color:C.dim,marginBottom:4}}>📍 {t.home_city}, {t.home_state}{t.willing_to_go?` · Goes to: ${t.willing_to_go}`:’’}</div>
{(t.available_from||t.available_to)&&<div style={{fontSize:11,color:C.dim,marginBottom:4}}>📅 {t.available_from||‘Now’}{t.available_to?` → ${t.available_to}`:’’}</div>}
{t.company&&<div style={{fontSize:11,color:C.dim,marginBottom:4}}>🏢 {t.company}</div>}
{t.notes&&<div style={{fontSize:11,color:C.faint,marginTop:4,lineHeight:1.6}}>{t.notes}</div>}
</div>
<div style={{flexShrink:0}}>
{driverEmail?
<div style={{textAlign:‘right’}}>
{t.phone&&<a href={`tel:${t.phone.replace(/[^0-9+]/g,'')}`} style={{display:‘block’,background:C.green,color:’#000’,borderRadius:7,padding:‘7px 12px’,textDecoration:‘none’,fontSize:12,fontWeight:700,marginBottom:6}}>📞 Call</a>}
{t.email&&<a href={`mailto:${t.email}`} style={{display:‘block’,background:C.surface,color:C.white,border:`1px solid ${C.border}`,borderRadius:7,padding:‘7px 12px’,textDecoration:‘none’,fontSize:12,fontWeight:700}}>✉ Email</a>}
</div>:
<div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:8,padding:‘10px’,textAlign:‘center’}}>
<div style={{fontSize:10,color:C.dim,marginBottom:6}}>Driver access required<br/>to see contact info</div>
<button onClick={()=>window.location.href=’?view=driver’} style={{background:C.orange,color:’#000’,border:‘none’,borderRadius:6,padding:‘6px 12px’,fontSize:11,fontWeight:700,cursor:‘pointer’,fontFamily:ff}}>Get Access</button>
</div>
}
</div>
</div>
...
// 📄 BOL GENERATOR
// ═══════════════════════════════════════════════════════════════════
function BOLGenerator({onBack}){
const [form,setForm]=useState({shipperName:’’,shipperPhone:’’,shipperAddress:’’,carrierName:’’,carrierPhone:’’,carrierMC:’’,pickupAddress:’’,deliveryAddress:’’,pickupDate:’’,deliveryDate:’’,freightDescription:’’,weight:’’,pieces:’’,specialInstructions:’’})
...
if(!form.shipperName||!form.carrierName||!form.pickupAddress){setMsg(‘Shipper name, carrier name, and pickup address required.’);return}
setLoading(true);setMsg(’’)
const res=await fetch(’/.netlify/functions/generate-bol’,{method:‘POST’,headers:{‘Content-Type’:‘application/json’},body:JSON.stringify(form)})
const data=await res.json()
if(data.bol){setBol(data.bol)}
else{setMsg(‘Failed to generate BOL. Try again.’)}
setLoading(false)
}

function printBOL(){window.print()}

const label=(l)=><div style={{fontSize:9,color:C.dim,textTransform:‘uppercase’,letterSpacing:0.8,marginBottom:3}}>{l}</div>
const inp2=(k,l,t=‘text’,span=1)=>(
<div style={{marginBottom:10,gridColumn:span===2?‘1/-1’:‘auto’}}>
{label(l)}
<input type={t} value={form[k]} onChange={e=>setForm(p=>({…p,[k]:e.target.value}))} style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:7,color:C.white,padding:‘8px 10px’,fontSize:12,width:‘100%’,outline:‘none’,fontFamily:ff,boxSizing:‘border-box’,colorScheme:‘dark’}}/>
</div>
)
...
<button onClick={()=>setBol(null)} style={{background:‘none’,border:‘none’,color:C.dim,cursor:‘pointer’,fontSize:13,fontFamily:ff}}>← New BOL</button>
<div style={{fontSize:16,fontWeight:900,color:C.white,flex:1}}>📄 BOL Generated</div>
<button onClick={printBOL} style={{background:C.orange,color:’#000’,border:‘none’,borderRadius:7,padding:‘8px 16px’,fontSize:12,fontWeight:700,cursor:‘pointer’,fontFamily:ff}}>🖨 Print / Save PDF</button>
</div>
<div id=“bol-print” style={{background:’#fff’,color:’#000’,borderRadius:10,padding:‘30px’,fontFamily:‘Arial,sans-serif’}}>
<div style={{display:‘flex’,justifyContent:‘space-between’,alignItems:‘flex-start’,marginBottom:20}}>
<div>
<div style={{fontSize:20,fontWeight:900,letterSpacing:1}}>BILL OF LADING</div>
<div style={{fontSize:12,color:’#555’,marginTop:2}}>POTENT LOADBOARD · A POTENT PRÄDƏKT® PLATFORM</div>
</div>
<div style={{textAlign:‘right’}}>
<div style={{fontSize:11,color:’#555’}}>BOL Number</div>
<div style={{fontSize:16,fontWeight:700,fontFamily:‘monospace’}}>{bol.bol_number}</div>
<div style={{fontSize:10,color:’#888’,marginTop:2}}>Generated: {new Date(bol.created_at).toLocaleDateString()}</div>
</div>
</div>
<div style={{display:‘grid’,gridTemplateColumns:‘1fr 1fr’,gap:‘0 30px’,borderTop:‘2px solid #000’,paddingTop:16}}>
{[[‘Shipper’,bol.shipper_name,bol.shipper_phone,bol.shipper_address],[‘Carrier’,bol.carrier_name,bol.carrier_phone,`MC#: ${bol.carrier_mc||'N/A'}`]].map(([role,name,phone,addr])=>(
<div key={role} style={{marginBottom:16}}>
<div style={{fontSize:10,fontWeight:700,textTransform:‘uppercase’,color:’#555’,marginBottom:4,borderBottom:‘1px solid #ddd’,paddingBottom:2}}>{role}</div>
<div style={{fontSize:13,fontWeight:700}}>{name}</div>
<div style={{fontSize:11,color:’#444’}}>{phone}</div>
<div style={{fontSize:11,color:’#444’}}>{addr}</div>
</div>
))}
</div>
<div style={{display:‘grid’,gridTemplateColumns:‘1fr 1fr’,gap:‘0 30px’,borderTop:‘1px solid #ddd’,paddingTop:12,marginBottom:12}}>
{[[‘Pickup Address’,bol.pickup_address],[‘Delivery Address’,bol.delivery_address],[‘Pickup Date’,bol.pickup_date||’—’],[‘Delivery Date’,bol.delivery_date||’—’]].map(([l,v])=>(
<div key={l} style={{marginBottom:10}}>
<div style={{fontSize:9,textTransform:‘uppercase’,color:’#888’,marginBottom:2}}>{l}</div>
<div style={{fontSize:12}}>{v}</div>
</div>
))}
</div>
<div style={{borderTop:‘1px solid #ddd’,paddingTop:12,marginBottom:16}}>
<div style={{fontSize:9,textTransform:‘uppercase’,color:’#888’,marginBottom:6}}>Freight Description</div>
<table style={{width:‘100%’,borderCollapse:‘collapse’,fontSize:11}}>
<thead><tr style={{background:’#f5f5f5’}}><th style={{padding:‘6px 8px’,textAlign:‘left’,border:‘1px solid #ddd’}}>Description</th><th style={{padding:‘6px 8px’,textAlign:‘center’,border:‘1px solid #ddd’}}>Weight</th><th style={{padding:‘6px 8px’,textAlign:‘center’,border:‘1px solid #ddd’}}>Pieces</th></tr></thead>
<tbody><tr><td style={{padding:‘8px’,border:‘1px solid #ddd’}}>{bol.freight_description}</td><td style={{padding:‘8px’,border:‘1px solid #ddd’,textAlign:‘center’}}>{bol.weight||’—’}</td><td style={{padding:‘8px’,border:‘1px solid #ddd’,textAlign:‘center’}}>{bol.pieces||’—’}</td></tr></tbody>
</table>
{bol.special_instructions&&<div style={{marginTop:8,fontSize:11,color:’#555’}}>Special Instructions: {bol.special_instructions}</div>}
</div>
<div style={{display:‘grid’,gridTemplateColumns:‘1fr 1fr’,gap:‘0 30px’,borderTop:‘2px solid #000’,paddingTop:16}}>
{[‘Shipper Signature’,‘Carrier Signature’].map(s=>(
<div key={s} style={{marginBottom:10}}>
<div style={{fontSize:9,textTransform:‘uppercase’,color:’#888’,marginBottom:20}}>{s}</div>
<div style={{borderBottom:‘1px solid #000’,marginBottom:4}}></div>
<div style={{fontSize:9,color:’#888’}}>Signature / Date</div>
</div>
))}
</div>
<div style={{fontSize:8,color:’#aaa’,textAlign:‘center’,marginTop:16,borderTop:‘1px solid #ddd’,paddingTop:10}}>
This BOL is generated by POTENT Loadboard as a platform convenience. POTENT Loadboard is not a party to this transaction and bears no liability. All fees final, no refunds. potentlogistics@pm.me · (770) 648-4228
</div>
</div>
</div>
)
...
<div style={{display:‘flex’,gap:10,alignItems:‘center’,marginBottom:16}}>
<button onClick={onBack} style={{background:‘none’,border:‘none’,color:C.dim,cursor:‘pointer’,fontSize:13,fontFamily:ff}}>← Back</button>
<div style={{fontSize:16,fontWeight:900,color:C.white}}>📄 Digital Bill of Lading</div>
</div>
<div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:10,padding:‘14px’,marginBottom:14,fontSize:11,color:C.dim,lineHeight:1.7}}>
Free to generate. BOL is watermarked with POTENT Loadboard. Print or save as PDF. <strong style={{color:C.orange}}>Both parties should sign a copy before freight moves.</strong>
</div>
<div style={{display:‘grid’,gridTemplateColumns:‘1fr 1fr’,gap:‘0 12px’}}>
{inp2(‘shipperName’,‘Shipper Name’)}
{inp2(‘shipperPhone’,‘Shipper Phone’,‘tel’)}
{inp2(‘shipperAddress’,‘Shipper Address’,‘text’,2)}
{inp2(‘carrierName’,‘Carrier Name’)}
{inp2(‘carrierPhone’,‘Carrier Phone’,‘tel’)}
{inp2(‘carrierMC’,‘Carrier MC#’)}
{inp2(‘pickupAddress’,‘Pickup Address’,‘text’,2)}
{inp2(‘deliveryAddress’,‘Delivery Address’,‘text’,2)}
{inp2(‘pickupDate’,‘Pickup Date’,‘date’)}
{inp2(‘deliveryDate’,‘Delivery Date’,‘date’)}
{inp2(‘freightDescription’,‘Freight Description’,‘text’,2)}
{inp2(‘weight’,‘Weight’)}
{inp2(‘pieces’,‘Pieces / Units’)}
{inp2(‘specialInstructions’,‘Special Instructions’,‘text’,2)}
</div>
{msg&&<div style={{color:C.red,fontSize:11,marginBottom:10}}>{msg}</div>}
<button onClick={generate} disabled={loading} style={{width:‘100%’,background:loading?C.surface:C.orange,color:loading?C.dim:’#000’,border:‘none’,borderRadius:8,padding:‘13px’,fontSize:14,fontWeight:700,cursor:loading?‘not-allowed’:‘pointer’,fontFamily:ff}}>
{loading?‘Generating…’:‘📄 Generate BOL’}
</button>
</div>
)
}
...
const ff=”‘DM Sans’,‘Segoe UI’,sans-serif”
const terms=[
[‘Platform Only’,‘POTENT Loadboard is a technology platform only. We connect shippers and carriers. We do not broker freight, arrange transportation, or guarantee any transaction between users.’],
[‘No Refunds’,‘All fees are final and non-refundable. You are paying for platform access only — not for any specific result, connection, or completed load. No exceptions.’],
[‘No Liability’,‘POTENT Loadboard is not responsible for any loss, damage, fraud, dispute, failed delivery, or injury arising from connections made through this platform. All transactions are strictly between the parties involved.’],
[‘Your Responsibility’,‘You are solely responsible for vetting, qualifying, and deciding to work with any user on this platform. POTENT does not verify any user, load, carrier, business, or freight description.’],
[‘Off-Platform Transactions’,‘Arranging transactions outside this platform after making contact through it violates these terms. Violating accounts will be permanently banned with no refund.’],
[‘Permanent Bans’,‘POTENT Loadboard reserves the right to permanently ban any account at any time for any reason including fraud, misrepresentation, TOS violations, or off-platform solicitation. No refund will be issued.’],
[‘Dispute Resolution’,‘Dispute resolution is a paid service ($49). POTENT reviews the case and issues a recommendation. It is non-binding and not legally enforceable. Non-refundable.’],
[‘Digital BOL’,‘BOLs generated through this platform are provided as a convenience. They are not verified legal documents. POTENT bears no liability for their use or enforceability.’],
[‘Contact’,‘potentlogistics@pm.me · (770) 648-4228 · Conyers, GA · POTENT PRÄDƏKT® · © 2026 ANTHONY EMMANUEL FIGUEROA MENDES®’],
]
return(
<div style={{maxWidth:700,margin:‘0 auto’,padding:‘20px 16px 60px’,fontFamily:ff}}>
<div style={{display:‘flex’,gap:10,alignItems:‘center’,marginBottom:20}}>
<button onClick={onBack} style={{background:‘none’,border:‘none’,color:C.dim,cursor:‘pointer’,fontSize:13,fontFamily:ff}}>← Back</button>
<div style={{fontSize:18,fontWeight:900,color:C.white}}>Terms of Service</div>
</div>
<div style={{fontSize:10,color:C.faint,marginBottom:20}}>Last updated: September 2026 · By using POTENT Loadboard you agree to these terms.</div>
{terms.map(([title,text],i)=>(
<div key={i} style={{marginBottom:18,borderLeft:`3px solid ${C.orange}`,paddingLeft:14}}>
<div style={{fontSize:13,fontWeight:700,color:C.white,marginBottom:4}}>{title}</div>
<div style={{fontSize:11,color:C.dim,lineHeight:1.8}}>{text}</div>
</div>
))}
</div>
)
